...
const init = (arr) => {
     ...
     */
    for (const [i, v] of Object.entries(depend)) {
        // i = index, v = parent index
    ...
    for (const [i, v] of Object.entries(leaves)) {
        ...
        v.addEventListener("mouseover", function (e) {
            ...
            while (cnode.getAttribute("dep") >= 0) {
                ...
                if (cnode.getAttribute("dep") < 1) {
                    break;
                }
                cnode = leaves[cnode.getAttribute("dep")];
                edges[j].firstElementChild.setAttribute(
                    "class",
                    "edge_highlight"
                ); // the edges
                edges[j].firstElementChild.nextElementSibling.setAttribute(
                    "class",
                    "edge_label_highlight"
                ); // bit of an inelegance for you
            }
        });
        v.addEventListener("mouseout", function (e) {
            ...
            while (cnode.getAttribute("dep") >= 0) {
                ...
                edges[j].firstElementChild.nextElementSibling.classList.remove(
                    "edge_label_highlight"
                ); // more inelegance
            }
        });
...
const connect = (leaves, i, j, y_base, lev = 0, taken = {}, fontsize) => {
    ...
    while (h in taken && intersect) {
        ...
        for (const [i, e] of Object.entries(taken[h])) {
            const sect = range(e).filter((value) =>
                range(
                    [s, t].sort(function (a, b) {
                        return a - b;
                    })
                ).includes(value)
            );
...
const edge = (s, l, y_base, h, t, id, left, fontsize, match, edgeID) => {
    ...
    */
    ...
    let lab = label(
        ...
        id,
        match
    ); //(y_base-7)-h seems a bit… esoteric. Let's fix this. Like, how to express 7?
    g.appendChild(lab);
    return g;
};
...
const node = (x, y, a, fontsize, uniq) => {
    ...
    g.appendChild(p);
    g.appendChild(
        txt(x - 2, y + fontsize - 2, w, lab, "mid" + id, fontsize, hit)
    );
    let x2 = x - w2_offset;
    g.appendChild(
        txt(x2, y + fontsize * 3, w2, tok, "tokm" + id, fontsize, hit)
    ); // YYY t_base
    return g;
};
